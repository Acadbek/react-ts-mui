...
const Modal = ({ brands, category, errorBrand }: ModalTypes) => {
  const dispatch = useDispatch();
  const totalSteps: number = 3;

  const { currentStep, next, prev, isFirstStep, isLastStep } = useSteps(totalSteps);
  ...
  return (
    <div className="transition-all">
      ...
      <form className="modal !bg-white !z-[99] absolute left-2/4 top-2/4 transform -translate-x-1/2 -translate-y-1/2 h-[574px] overflow-y-auto w-[500px] rounded-xl">
        <button onClick={handleNext}>next</button> - {currentStep}
